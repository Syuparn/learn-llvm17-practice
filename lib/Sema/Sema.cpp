...
void Sema::initialize() {
  ...
  IntegerType = new TypeDeclaration(
      CurrentDecl, llvm::SMLoc(), "INTEGER");
  BooleanType = new TypeDeclaration(
      CurrentDecl, llvm::SMLoc(), "BOOLEAN");
  TrueLiteral = new BooleanLiteral(true, BooleanType);
  FalseLiteral = new BooleanLiteral(false, BooleanType);
  TrueConst = new ConstantDeclaration(
      CurrentDecl, llvm::SMLoc(), "TRUE", TrueLiteral);
  FalseConst = new ConstantDeclaration(
      CurrentDecl, llvm::SMLoc(), "FALSE", FalseLiteral);
...
ModuleDeclaration *
Sema::actOnModuleDeclaration(llvm::SMLoc Loc,
                             llvm::StringRef Name) {
  return new ModuleDeclaration(CurrentDecl, Loc, Name);
}

void Sema::actOnModuleDeclaration(
    ModuleDeclaration *ModDecl, llvm::SMLoc Loc,
    llvm::StringRef Name, DeclList &Decls,
    StmtList &Stmts) {
...
void Sema::actOnVariableDeclaration(DeclList &Decls,
  ...
  if (TypeDeclaration *Ty =
          llvm::dyn_cast<TypeDeclaration>(D)) {
...
Sema::actOnProcedureDeclaration(llvm::SMLoc Loc,
                                llvm::StringRef Name) {
...
void Sema::actOnAssignment(StmtList &Stmts, llvm::SMLoc Loc,
                           Decl *D, Expr *E) {
...
void Sema::actOnProcCall(StmtList &Stmts, llvm::SMLoc Loc,
                         Decl *D, ExprList &Params) {
  if (auto *Proc =
          llvm::dyn_cast<ProcedureDeclaration>(D)) {
...
void Sema::actOnWhileStatement(StmtList &Stmts,
                               llvm::SMLoc Loc, Expr *Cond,
                               StmtList &WhileStmts) {
  if (!Cond)
    Cond = FalseLiteral;
...
void Sema::actOnReturnStatement(StmtList &Stmts,
                                llvm::SMLoc Loc,
                                Expr *RetVal) {
  auto *Proc =
      llvm::cast<ProcedureDeclaration>(CurrentDecl);
...
Expr *Sema::actOnSimpleExpression(Expr *Left, Expr *Right,
  ...
  if (IsConst && Op.getKind() == tok::kw_OR) {
    BooleanLiteral *L =
        llvm::dyn_cast<BooleanLiteral>(Left);
    BooleanLiteral *R =
        llvm::dyn_cast<BooleanLiteral>(Right);
...
Expr *Sema::actOnPrefixExpression(Expr *E,
  ...
  if (Op.getKind() == tok::minus) {
    ...
    if (llvm::isa<IntegerLiteral>(E) ||
        llvm::isa<VariableAccess>(E) ||
        llvm::isa<ConstantAccess>(E))
      Ambiguous = false;
    else if (auto *Infix =
                 llvm::dyn_cast<InfixExpression>(E)) {
      tok::TokenKind Kind =
...
Expr *Sema::actOnIntegerLiteral(llvm::SMLoc Loc,
...
Expr *Sema::actOnVariable(Decl *D) {
  ...
  if (auto *P =
          llvm::dyn_cast<FormalParameterDeclaration>(D))
    return new VariableAccess(P);
  if (auto *C = llvm::dyn_cast<ConstantDeclaration>(D)) {
    if (C == TrueConst)
      return TrueLiteral;
    if (C == FalseConst) {
      return FalseLiteral;
...
Decl *Sema::actOnQualIdentPart(Decl *Prev, llvm::SMLoc Loc,
                               llvm::StringRef Name) {
  if (!Prev) {
