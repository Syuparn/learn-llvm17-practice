...
llvm::Value *
CGProcedure::readLocalVariable( // NOLINT(misc-no-recursion)
...
llvm::Value *CGProcedure::
    readLocalVariableRecursive( // NOLINT(misc-no-recursion)
        llvm::BasicBlock *BB, Decl *Decl) {
...
llvm::Value *
CGProcedure::addPhiOperands( // NOLINT(misc-no-recursion)
...
llvm::Value *
CGProcedure::optimizePhi( // NOLINT(misc-no-recursion)
    llvm::PHINode *Phi) {
...
llvm::Value *
CGProcedure::emitInfixExpr( // NOLINT(misc-no-recursion)
    InfixExpression *E) {
...
llvm::Value *
CGProcedure::emitPrefixExpr( // NOLINT(misc-no-recursion)
    PrefixExpression *E) {
...
llvm::Value *
CGProcedure::emitExpr( // NOLINT(misc-no-recursion)
    Expr *E) {
  if (auto *Infix = llvm::dyn_cast<InfixExpression>(E)) {
    return emitInfixExpr(Infix);
  }
  if (auto *Prefix = llvm::dyn_cast<PrefixExpression>(E)) {
  ...
  if (auto *Const = llvm::dyn_cast<ConstantAccess>(E)) {
    return emitExpr(Const->getDecl()->getExpr());
  }
  if (auto *IntLit = llvm::dyn_cast<IntegerLiteral>(E)) {
    return llvm::ConstantInt::get(CGM.Int64Ty,
                                  IntLit->getValue());
  }
  if (auto *BoolLit = llvm::dyn_cast<BooleanLiteral>(E)) {
...
void CGProcedure::emitStmt( // NOLINT(misc-no-recursion)
    IfStatement *Stmt) {
...
void CGProcedure::emit( // NOLINT(misc-no-recursion)
