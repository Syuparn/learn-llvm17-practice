...
bool Parser::parseBlock( // NOLINT(misc-no-recursion)
...
bool Parser::parseDeclaration( // NOLINT(misc-no-recursion)
    DeclList &Decls) {
  auto _errorhandler = [this] {
    return skipUntil(tok::kw_BEGIN, tok::kw_CONST,
...
bool Parser::
    parseProcedureDeclaration( // NOLINT(misc-no-recursion)
...
bool Parser::
    parseStatementSequence( // NOLINT(misc-no-recursion)
        StmtList &Stmts) {
  auto _errorhandler = [this] {
...
bool Parser::parseStatement( // NOLINT(misc-no-recursion)
    StmtList &Stmts) {
  auto _errorhandler = [this] {
...
bool Parser::parseIfStatement( // NOLINT(misc-no-recursion)
    StmtList &Stmts) {
  auto _errorhandler = [this] {
    return skipUntil(tok::semi, tok::kw_ELSE, tok::kw_END);
  };
...
bool Parser::
    parseWhileStatement( // NOLINT(misc-no-recursion)
        StmtList &Stmts) {
  auto _errorhandler = [this] {
    return skipUntil(tok::semi, tok::kw_ELSE, tok::kw_END);
  };
...
bool Parser::parseExpList( // NOLINT(misc-no-recursion)
    ExprList &Exprs) {
  auto _errorhandler = [this] {
    return skipUntil(tok::r_paren);
  };
...
bool Parser::parseExpression( // NOLINT(misc-no-recursion)
    Expr *&E) {
  auto _errorhandler = [this] {
...
bool Parser::
    parseSimpleExpression( // NOLINT(misc-no-recursion)
        Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(
...
bool Parser::parseTerm( // NOLINT(misc-no-recursion)
    Expr *&E) {
  auto _errorhandler = [this] {
...
bool Parser::parseFactor( // NOLINT(misc-no-recursion)
    Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(
...
bool Parser::parseIdentList(IdentList &Ids) {
  ...
  if (expect(tok::identifier))
    return _errorhandler();
  Ids.push_back(std::pair<llvm::SMLoc, llvm::StringRef>(
      Tok.getLocation(), Tok.getIdentifier()));
  advance();
  while (Tok.is(tok::comma)) {
    advance();
